...
const TOPIC_VALIDATION = 'iot/M1Miage2022'
...
async function v0(){
    ...
    const mongoUri = `mongodb+srv://remi:${passwordMongoDb}@cluster0.j8znq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`; //URL de connection		
	...
	client_mqtt.on('connect', function () {
	   ...
	   /client_mqtt.subscribe(TOPIC_TEMP, function (err) {
		...
		client_mqtt.subscribe(TOPIC_VALIDATION, function (err) {
			if (!err) {
				//client_mqtt.publish(TOPIC_TEMP, 'Hello mqtt')
				console.log('Node Server has subscribed to ', TOPIC_VALIDATION);
			}
		})
	})
	...
	client_mqtt.on('message', function (topic, message) {
		...
		switch(topic.toString()) {
			case TOPIC_VALIDATION :
				...
				if(dbo.collection('esps').find({ "ident": message.ident }).count() == 0) {
					dbo.collection('esps').insertOne({surname :'',ident :message.ident})
				}
				if (message.status && message.info){
					var time = new Date().toLocaleString("sv-SE", {timeZone: "Europe/Paris"});
					var new_data_entry = { 
						date: time,
						status: message.status,
						info: message.info
					};
					dbo.collection('M1Miage2022').insertOne(new_data_entry)
				}
				console.log("Nouveau message sur le topic Validation: ", message);
			break
			case TOPIC_LIGHT :
...
app.get('/espsConfig', function (req, res) {
    res.sendFile(path.join(__dirname + '/ui_lucioles_params.html'));
});

app.get('/map', function (req, res) {
    res.sendFile(path.join(__dirname + '/ui_lucioles_map.html'));
});

app.get('/esps',function (req, res) {
	dbo.collection('esps').find({}).toArray(function(err, result) {
		//if (err) throw err;
		console.log(result);
		res.json({result})
	  })
...
app.get('/M1Miage2022',function (req, res) {
	dbo.collection('M1Miage2022').aggregate([
		{
			$sort: { "date": 1 }
		},
		{
		    
			$group : { _id: "$info.ident" ,loc : { "$first" : "$info.loc"}, temperature : { "$first" : "$status.temperature"}}
		}
	]).toArray(function(err, result) {
		let filtred = result.filter(esp => {
			if(esp.hasOwnProperty('loc')) {
				if(esp.loc != null && esp.loc.hasOwnProperty('lat') && esp.loc.hasOwnProperty('lgn')) {
					return true
				}
			} else {
				return false
...
app.put('/esps/:id', function (req, res) {
	console.log(req.body)
	if(req.body.surname) {
    	dbo.collection('esps').updateOne({'_id':ObjectId(req.params.id)}, {$set : {surname : req.body.surname}},function(err, resu) {
			console.log(resu)
			res.json({message : "ok"})
		  })
	} else {
